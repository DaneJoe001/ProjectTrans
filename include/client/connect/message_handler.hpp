...
public:
     ...
     * @param message 消息
     */
    static std::vector<uint8_t> build_test_request(const DaneJoe::UrlInfo url_info,const std::string& message);
    /**
     * @brief 构建上传消息
     * @param file_info 文件信息
     * @return 构建后的上传消息
     */
    static std::vector<uint8_t> build_upload_request(const DaneJoe::UrlInfo url_info,const ClientFileInfo& file_info);
    /**
     * @brief 构建下载消息
     * @param path 下载路径
     * @return 构建后的下载消息
     */
    static std::vector<uint8_t> build_download_request(const DaneJoe::UrlInfo url_info,const std::string& account,const std::string& password);
    /**
     * @brief 构建块请求
     * @param block_request_info 块请求信息
     * @return 构建后的块请求
     */
    static std::vector<uint8_t> build_block_request(const DaneJoe::UrlInfo url_info,const BlockRequestInfo& block_request_info);
};
}