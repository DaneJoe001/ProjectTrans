...
DaneJoe::EpollEventLoop::EpollEventLoop(std::unique_ptr<PosixServerSocket> server_socket, std::unique_ptr<ISocketContextCreator> context_creator)
{
    ...
    {
        ADD_DIAG_ERROR("network", "Create epoll failed: server socket is null or invalid");
        return;
    }
    // 服务器套接字
    ...
    {
        ADD_DIAG_ERROR("network", "Create epoll failed: set server socket non blocking failed");
        stop();
        return;
    }
    // 业务上下文创建器
    ...
    if (m_epoll_fd < 0)
    {
        ADD_DIAG_ERROR("network", "Create epoll failed: epoll_create1 failed");
        stop();
        return;
    }
    // 创建并初始化epoll_event结构
...
bool DaneJoe::EpollEventLoop::add_socket(std::unique_ptr<IClientSocket> socket, EventLoopEventType type)
{
    // 检查自身是否有效
    if (!is_valid())
    {
        ADD_DIAG_ERROR("network", "Add socket failed: epoll is invalid");
        return false;
    }
    // 检查套接字是否为空
    if (!socket)
    {
        ADD_DIAG_ERROR("network", "Add socket failed: socket is null");
        return false;
    }
    // 检查套接字是否有效
    if (!socket->is_valid())
    {
        ADD_DIAG_ERROR("network", "Add socket failed: socket is invalid");
        return false;
    }
    // 设置套接字需要关注的事件
    ...
    if (ret < 0)
    {
        ADD_DIAG_ERROR("network", "Add socket failed: epoll_ctl add failed: {}", std::strerror(errno));
        socket->close();
        return false;
    }
    // 初始化对应的接收缓存
    int32_t socket_id = socket->get_id();
    m_recv_buffers[socket_id] = std::make_shared<DaneJoe::MpmcBoundedQueue<uint8_t>>(4096);
    m_send_buffers[socket_id] = std::make_shared<DaneJoe::MpmcBoundedQueue<uint8_t>>(4096);
...
void DaneJoe::EpollEventLoop::remove_socket(int32_t socket_id)
{
    // 检查自身是否有效
    if (!is_valid())
    {
        ADD_DIAG_ERROR("network", "Remove socket failed: epoll is invalid");
        return;
    }
    // 获取套接字迭代器
    ...
    {
        ADD_DIAG_WARN("network", "Remove socket skipped: socket {} not found", socket_id);
        return;
    }
    // 获取套接字
    ...
    if (ret < 0)
    {
        ADD_DIAG_ERROR("network", "Remove socket failed: epoll_ctl del failed: {}", std::strerror(errno));
    }
...
void DaneJoe::EpollEventLoop::run()
{
    // 检查自身是否有效
    if (!is_valid())
    {
        ADD_DIAG_ERROR("network", "Run loop failed: epoll is invalid");
        return;
    }
    // 更新运行标志
...
bool DaneJoe::EpollEventLoop::is_valid()
{
    // 返回标识符是否大于等于0
    return m_epoll_fd >= 0;
}

uint32_t DaneJoe::EpollEventLoop::to_epoll_events(EventLoopEventType type)
{
    uint32_t event = 0;

    if (is_event_valid(type & EventLoopEventType::Readable))
    {
        // 可读事件
        event |= EPOLLIN;
    }
    if (is_event_valid(type & EventLoopEventType::Writable))
    {
        // 可写事件
        event |= EPOLLOUT;
    }
    if (is_event_valid(type & EventLoopEventType::Priority))
    {
        // 优先级事件
        event |= EPOLLPRI;
    }
    if (is_event_valid(type & EventLoopEventType::Disconnected))
    {
        // 断开事件
        event |= EPOLLHUP;
    }
    if (is_event_valid(type & EventLoopEventType::EdgeTriggered))
    {
        // 边缘触发
        event |= EPOLLET;
    }
    if (is_event_valid(type & EventLoopEventType::OneShot))
    {
        // 单次触发
        event |= EPOLLONESHOT;
    }
    if (is_event_valid(type & EventLoopEventType::Error))
    {
        // 错误事件
        event |= EPOLLERR;
    }
    if (is_event_valid(type & EventLoopEventType::PeerClosed))
    {
        // 对端关闭
        event |= EPOLLRDHUP;
    }
...
void DaneJoe::EpollEventLoop::acceptable_event()
{
    // 先检查服务器套接字是否不为空且有效
    if (!m_server_socket || !m_server_socket->is_valid())
    {
        ADD_DIAG_ERROR("network", "Accept client failed: server socket is null or invalid");
        return;
    }
    ...
    while (true)
    {
        ...
        if (!client)
        {
            ...
            ADD_DIAG_ERROR("network", "Accept client failed: {}", std::strerror(errno));
            break;
        }
        // 确保设置客户端套接字非阻塞
        ...
        if (!is_set_non_blocking)
        {
            ADD_DIAG_WARN("network", "Accept client skipped: set non blocking failed");
            client->close();
            continue;
        }
        ...
        ADD_DIAG_TRACE("network", "Accept client succeeded");
        continue;
    }
}

void DaneJoe::EpollEventLoop::readable_event(int32_t socket_id)
{
    // 检查当前的映射表中是否存在该socket
    auto socket_iter = m_sockets.find(socket_id);
    if (socket_iter == m_sockets.end())
    {
        ADD_DIAG_WARN("network", "Handle readable event skipped: socket {} not found", socket_id);
        return;
    }
    // 检查套接字是否有效
    ...
    if (!data.empty())
    {
        ADD_DIAG_TRACE("network", "Readable event: from socket {}, data size {}", socket_id, data.size());
    }
    // 检查读取的长度
    ...
    while (pushed_count < data_length)
    {
        ...
        if (rest_space == 0)
        {
            ADD_DIAG_WARN("network", "Readable event: recv buffer is full");
            /// @todo 后续考虑加入超时机制
...
void DaneJoe::EpollEventLoop::writable_event(int32_t socket_id)
{
    ...
    {
        ADD_DIAG_WARN("network", "Handle writable event skipped: socket {} not found", socket_id);
        return;
    }
    ...
    if (data_optional.has_value())
    {
        ...
        {
            ADD_DIAG_TRACE("network", "Writable event: to socket {}, data size {}", socket_id, send_data.size());
        }
        // 发送数据
