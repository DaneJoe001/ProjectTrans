 ...
 * @version 0.2.0
 * @date 2025-12-17
 * @details 定义日志系统的基础配置与枚举：LogLevel、LoggerConfig、LogOutputSetting。
 *          这些类型用于控制日志等级过滤、输出目标（文件/控制台）、以及输出格式中包含哪些元信息。
 */
#pragma once
...
namespace DaneJoe
{
     ...
     * @brief 日志级别枚举
     * @details 用于控制日志过滤与标记。
     *          日志输出通常以“消息级别 >= 配置级别”为条件（具体比较规则由实现决定）。
     */
    enum class LogLevel
    {
        /// @brief 跟踪级别
     ...
     * @param level 日志级别
     * @return 对应的输出标志
     * @details 用于输出更紧凑的级别标识（例如 "I"/"W" 等），具体映射由实现决定。
     */
     ...
     * @struct LoggerConfig
     * @brief 日志配置结构体
     * @details 用于配置日志器的输出目标与过滤策略。
     *          - console_level/file_level：控制台/文件输出的过滤级别
     *          - enable_console/enable_file/enable_async：是否启用对应输出与异步模式
     *          - max_file_size/max_file_num/use_daily_log：文件滚动/切分策略（具体行为由实现决定）
     */
    struct LoggerConfig
    {
        /// @brief 日志名称
     ...
     * @brief 日志输出设置结构体
     * @details 控制日志头部/上下文信息是否输出。
     *          这些开关通常影响 ILogger::get_header() 生成的内容。
     */
    struct LogOutputSetting
    {
        /// @brief 是否启用时间
