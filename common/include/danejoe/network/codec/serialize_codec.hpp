 ...
 * @version 0.2.0
 * @date 2025-12-17
 * @details 定义 SerializeCodec，用于构建/解析基于 SerializeHeader 与 SerializeField 的序列化消息。
 *          该类支持将基本类型、字符串、数组以及字典式对象（ISerializeDictionary）编码到字节流中，并从字节流解析字段映射。
 */
#pragma once
...
namespace DaneJoe
{
     ...
     * @brief 序列化类
     * @details 提供序列化构建与反序列化解析能力。
     * @note 序列化到字节流的数据默认使用大端（网络字节序）。
     * @note 存储到 SerializeField 结构体中的 value 默认为本地字节序；写入字节流时转换为网络字节序。
     * @note 内部在序列化前通常不会对字段长度进行严格检查，调用方应确保数据与配置约束一致。
    ...
    public:
         ...
         * @param data_name 字段名称
         * @return 返回当前对象进行递归调用
         * @details 当前版本尚未实现对 std::map 的序列化，调用将被忽略并直接返回 *this。
         */
