 ...
 * @version 0.2.0
 * @date 2025-12-04
 * @details 定义 ScatterPointRange<T>，用于表达由离散点组成的“枚举范围”。
 *          内部使用 std::set<T> 存储散点，具有自动去重与有序性。
 */
#pragma once
...
namespace DaneJoe
{
     ...
     * @brief 散点范围
     * @tparam T 散点类型
     * @details 以 std::set<T> 表达散点集合：
     *          - 插入时自动去重
     *          - 元素按严格弱序排序（由 T 的比较规则决定）
     *          适用于对有限离散取值的包含性判断与集合合并。
     */
    template<typename T>
    class ScatterPointRange
    {
    public:
         ...
         * @details 以 "<variable_name>: {a, b, c}" 形式输出散点集合。
         */
