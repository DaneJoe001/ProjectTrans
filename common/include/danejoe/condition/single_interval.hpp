 ...
 * @version 0.2.0
 * @date 2025-12-04
 * @details 定义 SingleInterval<T>，用于表达单个连续区间（可包含无穷端点与开/闭端点）。
 *          区间端点由 EndpointRange<T> 表示，并提供区间关系判断与集合运算（交/并/差）等工具方法。
 */
#pragma once
...
namespace DaneJoe
{
    /**
     * @brief 区间状态
     * @details 用于描述区间端点受限情况：
     *          - Empty：空区间
     *          - Whole：全区间 (-∞,+∞)
     *          - FixedLeft：左端点有限、右端点无穷
     *          - FixedRight：左端点无穷、右端点有限
     *          - FixedBoth：左右端点皆有限
     */
     ...
     * @tparam T 区间元素类型
     * @note 不含边界方向
     * @details 该结构体仅表达端点的值与开闭性，不包含左右端点方向信息。
     */
    template <typename T>
     ...
     * @details 区间由左右端点组成：
     *          - 左端点为 BoundarySide::Left
     *          - 右端点为 BoundarySide::Right
     *          端点值为 std::nullopt 表示无穷端点（-∞ 或 +∞）。
     *          该类型支持区间关系判断（相交/相邻/包含等）以及集合运算（交/并/差）。
     */
    template <typename T>
    ...
    public:
        /**
         * @brief 默认构造函数为空集
         * @details 默认构造区间为 (0,0)（左右端点均为开端点），因此其为 Empty。
         */
         ...
         * @param point 端点
         * @details 若 point 为左端点则区间为 [point,+∞) / (point,+∞) 形式；
         *          若 point 为右端点则区间为 (-∞,point] / (-∞,point) 形式。
         */
         ...
         * @note 若左右端点均为有限值且 left.m_value > right.m_value，则会交换左右边界。
         */
        ...
        /**
         * @brief 初始化区间状态
         * @details 根据左右端点是否为无穷端点与是否构成空集，计算 IntervalState。
         */
        void init_state()
        {
         ...
         * @return true 相邻
         * @return false 不相邻
         * @details 两区间相邻指两者不相交且端点相接（开闭性互补）。
         */
         ...
         * @return true 不相交
         * @return false 相交
         * @details 当区间相邻时返回 false（相邻不视为不相交）。
         */
         ...
         * @note 差集为空时，返回单个空区间（Empty）。
         * @details 返回当前区间减去 other 后的区间集合（可能为 0/1/2 段）。
         */
         ...
         * @details 若两区间不相交或相邻，则返回 std::nullopt。
         */
         ...
         * @details 当两区间可合并（不分离）时返回并集；否则返回 std::nullopt。
         */
        ...
        /**
         * @brief 获取整数区间
         * @return 若 T 为整数类型则返回对应的 SingleInterval<int64_t>；否则返回 std::nullopt
         * @details 用于将整数区间统一提升为 int64_t 表示，便于后续统一处理。
         */
        std::optional<SingleInterval<int64_t>> get_integer_interval();
        /**
         * @brief 获取左端点
         * @return 左端点（BoundarySide::Left）
         */
    ...
    /**
     * @copydoc SingleInterval::get_integer_interval
     */
    template<typename T>
