...
namespace DaneJoe
{
     ...
     * @note 当前实现主要用于反序列化/类型派发：仅对内置算术类型、std::string、std::vector<uint8_t>、
     *       std::nullptr_t 以及 C 字符串（经 std::decay 后为 const char*）做显式映射。
     * @note 未覆盖的类型统一返回 DataType::Unknown。
     */
    template <class T>
     ...
     * @note 仅支持基础数据类型
     * @param type 数据类型枚举
     * @return uint32_t 数据类型长度（字节数）
     * @note 变长类型（如 String/ByteArray/Map/Array/Dictionary 等）返回 0。
     */
