...
namespace DaneJoe
{
     ...
     * @tparam T 待判断的类型
     * @note 若为 std::chrono::duration<Rep, Period> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<class T>
     ...
     * @note 若为 std::chrono::time_point<Clock, TimeType> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<class T>
     ...
     * @tparam T 表示时间类型（如 int、long long 等）
     * @note 若为 std::chrono::duration<T, std::micro> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<typename T>
     ...
     * @tparam T 表示时间类型（如 int、long long 等）
     * @note 若为 std::chrono::duration<T, std::milli> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<typename T>
     ...
     * @tparam T 表示时间类型（如 int、long long 等）
     * @note 若为 std::chrono::duration<T, std::nano> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<typename T>
     ...
     * @tparam T 表示时间类型（如 int、long long 等）
     * @note 若为 std::chrono::duration<T, std::ratio<1>> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<typename T>
     ...
     * @note 若为 std::basic_string_view<CharT, Traits> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<class T>
     ...
     * @note 若为 const char* 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<class T>
     ...
     * @note 若为 std::pair<Key, Value> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<class T>
     ...
     * @tparam T 待判断的类型
     * @note 若为 std::optional<U> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<class T>
     ...
     * @note 若为 std::variant<Args...> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<class T>
     ...
     * @tparam Args... 表示元组类型（如 int、std::string 等）
     * @note 若为 std::tuple<Args...> 形式，则 Trait<T>::value 为 true，否则为 false。
     */
    template<class T>
